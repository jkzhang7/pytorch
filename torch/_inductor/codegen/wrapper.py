...
def _constants_offloaded_to_disk() -> Iterator[None]:
    ...
    # serialization reads them back -- is specific to AOT, as is the config
    # this is gated on.
    accelerator = (
    ...
    if (
        ...
        and getattr(V.graph, "aot_mode", False)
        ...
        and not getattr(V.graph, "is_const_graph", False)
    ):
        # Constants can span devices; spill only the current one's, so the
        # capacity the fraction is measured against matches what is freed.
        device = torch.device(
...
def _spill_constants(
    ...
    try:
        ...
        log.exception(
            "Failed to free constant %d of %d after spilling; restoring", freed, len(targets)
        )
