...
def get_kernel_py_stacks() -> Mapping[int, str]:
    ...
    aliases for node types without source ids. Before instantiation, keys are
...
def dump_kernel_py_stacks(path: str | PathLike[str]) -> None:
    ...
    Save the currently recorded CUDA graph Python launch stacks as JSON.
    ...
    Args:
        path (str or os.PathLike): Destination file, overwritten if it exists.
    ...
    Example::
        ...
        >>> torch.cuda.graph_annotations.dump_kernel_py_stacks("graph_stacks.json")
    """
    import json
