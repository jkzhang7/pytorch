...
_torchcomms_creation_lock = threading.Lock()
...
def _create_torchcomms_backend(
    ...
    *,
    ...
    # TorchComms currently discovers rank and size from the environment, and
    # new_comm releases the GIL during communicator initialization. Serialize
    # the temporary process-wide mutation across concurrent group creation.
    with _torchcomms_creation_lock:
        saved_rank_size = (
...
def _get_compatible_split_source(
    ...
    """
    split_from = _get_split_source(pg)
    split_device = pg.bound_device_id
    if split_from is None or split_device is None:
        return None
...
def _new_process_group_helper(
    ...
    if is_initialized() and _get_default_group().bound_device_id:
        split_from = _get_compatible_split_source(_get_default_group(), backend)
    else:
        split_from = None
