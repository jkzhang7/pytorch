...
Stacks are captured for all annotatable nodes, including work outside
``mark_kernels`` scopes. They contain live user frames on the launching thread;
C++ autograd nodes do not recover their forward Python stacks.
...
with torch.autograd.grad_mode.set_multithreading_enabled(False):
    with torch.cuda.graph(
        ...
        y = x @ x.t()
dump_kernel_py_stacks("graph_stacks.json")
```
...
with {func}`~torch.cuda.graph_annotations.dump_kernel_py_stacks`. The JSON file
...
retains capture ids (and requires CUPTI and driver 13.4). Nodes without source
