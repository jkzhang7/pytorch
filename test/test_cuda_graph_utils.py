...
class TestCuptiAnnotationBackend(TestCase):
    ...
    def test_py_stacks_cleanup(self, finish):
        import gc
        ...
        with torch.cuda.graph(live, enable_annotations=True, annotation_config=config):
            x.add_(1)
        saved = dict(get_kernel_py_stacks())
        ...
        if finish == "capture_end_error":
            ...
            with torch.cuda.graph(g, enable_annotations=True, annotation_config=config):
                x.add_(1)
            self.assertGreater(len(get_kernel_py_stacks()), len(saved))
            if finish == "reset":
                g.reset()
            else:
                del g
                gc.collect()
        self.assertEqual(dict(get_kernel_py_stacks()), saved)

    @parametrize("failure", ["edge_walk", "subscription", "autograd", "tools_id"])
    def test_py_stacks_require_cupti(self, failure):
        ...
        with ExitStack() as ctx:
            if failure == "edge_walk":
                ...
                error, message = RuntimeError, "requires cudaGraphNodeGetToolsId"
